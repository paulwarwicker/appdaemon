...
class AutomationLib(Hass):
    ...
    def initialize(self):
        # Grab defaults from config

        self.debug = self.args('debug', False)
        self.verbose = self.args('verbose', False)
        self.testing = self.args('testing', False)
        self.trace = self.args('trace', False)
...
# -----------------------------------------------------------------------------------
    ...
    def get_testing(self) -> bool:
        ...
        # return self.get_state('input_boolean.testing') == 'on'
        return self.testing
...
# -----------------------------------------------------------------------------------
    ...
    def log_function_name(self, start: bool=True, separator: bool=True, notification: bool=False, stacktrace: bool=False) -> None:
        """log function name as log message"""

        trace = self.get_state("input_boolean.trace") == "on"

        name = inspect.currentframe().f_back.f_code.co_name
        prefix = '>>>' if start else '<<<'

        if notification:
            self.call_service('announcer/notification', message=f'{prefix} {name}', type='desktop')
        ...
        if trace:
            if separator:
                dashes = '-' * (80 - len(name))
                self.log(f'{prefix} {name} {dashes}',level='WARNING')
            else:
...
# -----------------------------------------------------------------------------------
    ...
    def is_twilight(self) -> bool:
        ...
        if self.get_verbose_debug():
            ...
            self.log(f'{t} twilight', level='DEBUG')
...
# -----------------------------------------------------------------------------------
    ...
    def is_night(self) -> bool:
        ...
        if self.get_verbose_debug():
            ...
            self.log(f'{t} night', level='DEBUG')
...
# -----------------------------------------------------------------------------------
    ...
    def is_below_horizon(self) -> bool:
        ...
        if self.get_verbose_debug():
            ...
            self.log(f'{t} below horizon', level='DEBUG')
